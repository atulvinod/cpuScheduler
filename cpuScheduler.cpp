// CPP program to implement custom cpu scheduler
/* Made by Atul Vinod*/
...
// Set priority according to the burst time given by the user
void setPriority(Process array[]){
    float a  = 1;
    for(int i=0;i<(sizeof(array)/sizeof(Process));i++){
        array[i].priority = (int)(a/float(array[i].burstTime)*100);
    }
}
